...
void* jacobi2d_right(void *args) {
    Halo_Right *This = (Halo_Right*) args;
    ...
    for (int i = 0; i < This->mesh.size_y; i++, hd++) {
        *hd = *hd + ((*(This->mesh.mesh))[i][This->mesh.size_x - 1] * 0.25);
    }
...
/**
 * \brief Construtor do Halo_Right, sobrepondo-o com o vizinho de rank rank_neighbor.
 * 
 * @param [in] halo_size      Tamanho do Halo_Right.
 ...
 * @return
 */
Halo_Right::Halo_Right(int halo_size, int num_halos, struct mesh_t mesh,
...
/**
 * \brief Aloca o vetor bidimensional que conterá os dados do Halo_Right.
 * @return Primeira posição de memória do espaço alocado.
 */
void Halo_Right::alloc(void) {
...
void Halo_Right::set_iterators(void) {
    if (this->get_num_halos() <= 1) {
        return;
    }
...
/**
 * \brief Sobrepõe o Halo_Right com um vizinho.
 * 
 * @param [in] rank_neighbor  Rank do vizinho.
 * @param [in] direction_tag  Tag da mensagem.
 * @param [in] comm           Comunicador MPI.
 */
void Halo_Right::set_neighbor_info(int rank_neighbor, int direction_tag, MPI_Comm comm) {
 ...
 */
int Halo_Right::get_neighbor_rank(void) {
    return this->rank_neighbor;
}

/**
 * \brief Devolve o tamanho do Halo_Right.
 * @return Tamanho do Halo_Right.
 */
int Halo_Right::get_halo_size(void) {
 ...
 */
int Halo_Right::get_num_halos(void) {
 ...
 * mensagem do vizinho informado ou no momento da instanciação do Halo_Right
 ...
 */
bool Halo_Right::init_update(void) {
    ...
    if (this->transfer()) {
        ...
        pthread_create(&(this->thread_id), &(this->thread_attr), jacobi2d_right, this);
        return false;
    }
...
/**
 * \brief Acessa as posições da Halo_Right, a fim de possibilitar o cálculo das
 *        bordas dos subdomínios.
 * 
 * As posições acessadas são sempre as últimas do array bidimensional que
 * armazena os dados da Halo_Right. Se houverem múltiplas Halos e for necessário o
 * cálculo desses pontos sobrepostos, é apenas a Halo_Right mais próxima da borda
 * do subdomínio que deverá ser utilizada pelo processo.       
 * 
 * @param [in] i  Índice.
 * @return Referência da posição do vetor.
 */
double& Halo_Right::operator[](int i) {
    return this->data_source[this->start_row][this->start_col + i];
}

/**
 * \brief Acessa as posições da Halo_Right em um contexto const.
 * 
 * As posições acessadas são sempre as últimas do array bidimensional que
 * armazena os dados da Halo_Right. Se houverem múltiplas Halos e for necessário o
 * cálculo desses pontos sobrepostos, é apenas a Halo_Right mais próxima da borda
 ...
 */
double Halo_Right::operator[](int i) const {
    return this->data_source[this->start_row][this->start_col + i];
}

/**
 * \brief Faz o dump dos dados da Halo_Right para o arquivo de nome filename.
 * 
 * @param [in] filename  Nome do arquivo.
 */
void Halo_Right::print_to_file(const char *filename) {
    if (this->rank_neighbor == MPI_PROC_NULL) {
        return;
    }
 ...
 * @param [in] filename  Nome do arquivo.
 */
void Halo_Right::print_iter_to_file(const char *filename) {
    if (this->rank_neighbor == MPI_PROC_NULL) {
        return;
    }
...
void Halo_Right::print_info(void) {
...
void Halo_Right::print_buff(double **buff, int m, int n) {
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
