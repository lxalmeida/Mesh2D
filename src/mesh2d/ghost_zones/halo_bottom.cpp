...
void* jacobi2d_bottom(void *args) {
    Halo_Bottom *This = (Halo_Bottom*) args;

    for (int halo = 0; halo < This->num_iterators_ctrl; halo++) {
    ...
    for (int i = 0; i < This->mesh.size_y; i++, hd++) {
        *hd = *hd + ((*(This->mesh.mesh))[This->mesh.size_y - 1][i] * 0.25);
    }
...
/**
 * \brief Construtor do Halo_Bottom, sobrepondo-o com o vizinho de rank rank_neighbor.
 * 
 * @param [in] halo_size      Tamanho do Halo_Bottom.
 ...
 * @return
 */
Halo_Bottom::Halo_Bottom(int halo_size, int num_halos, struct mesh_t mesh,
...
/**
 * \brief Aloca o vetor bidimensional que conterá os dados do Halo_Bottom.
 * @return Primeira posição de memória do espaço alocado.
 */
void Halo_Bottom::alloc(void) {
...
void Halo_Bottom::set_iterators(void) {
    if (this->get_num_halos() <= 1) {
...
/**
 * \brief Sobrepõe o Halo_Bottom com um vizinho.
 * 
 * @param [in] rank_neighbor  Rank do vizinho.
 * @param [in] direction_tag  Tag da mensagem.
 * @param [in] comm           Comunicador MPI.
 */
void Halo_Bottom::set_neighbor_info(int rank_neighbor, int direction_tag, MPI_Comm comm) {
    // Rank do vizinho
    if (rank_neighbor < 0) {
 ...
 */
int Halo_Bottom::get_neighbor_rank(void) {
    return this->rank_neighbor;
}

/**
 * \brief Devolve o tamanho do Halo_Bottom.
 * @return Tamanho do Halo_Bottom.
 */
int Halo_Bottom::get_halo_size(void) {
 ...
 */
int Halo_Bottom::get_num_halos(void) {
 ...
 * mensagem do vizinho informado ou no momento da instanciação do Halo_Bottom
 ...
 */
bool Halo_Bottom::init_update(void) {
    ...
    if (this->transfer()) {
        ...
        pthread_create(&(this->thread_id), &(this->thread_attr), jacobi2d_bottom, this);
        return false;
    }
...
bool Halo_Bottom::transfer(void) {
    if (this->rank_neighbor == MPI_PROC_NULL || this->h_source == NULL) {
        return true;
    }

    return (this->num_iterators_ctrl <= 0);
}
...
/**
 * \brief Acessa as posições da Halo_Bottom, a fim de possibilitar o cálculo das
 *        bordas dos subdomínios.
 * 
 * As posições acessadas são sempre as últimas do array bidimensional que
 * armazena os dados da Halo_Bottom. Se houverem múltiplas Halos e for necessário o
 * cálculo desses pontos sobrepostos, é apenas a Halo_Bottom mais próxima da borda
 * do subdomínio que deverá ser utilizada pelo processo.       
 * 
 * @param [in] i  Índice.
 * @return Referência da posição do vetor.
 */
double& Halo_Bottom::operator[](int i) {
    return this->data_source[this->start_row][this->start_col + i];
}

/**
 * \brief Acessa as posições da Halo_Bottom em um contexto const.
 * 
 * As posições acessadas são sempre as últimas do array bidimensional que
 * armazena os dados da Halo_Bottom. Se houverem múltiplas Halos e for necessário o
 * cálculo desses pontos sobrepostos, é apenas a Halo_Bottom mais próxima da borda
 ...
 */
double Halo_Bottom::operator[](int i) const {
    return this->data_source[this->start_row][this->start_col + i];
}

/**
 * \brief Faz o dump dos dados da Halo_Bottom para o arquivo de nome filename.
 * 
 * @param [in] filename  Nome do arquivo.
 */
void Halo_Bottom::print_to_file(const char *filename) {
    if (this->rank_neighbor == MPI_PROC_NULL) {
        return;
    }
 ...
 * @param [in] filename  Nome do arquivo.
 */
void Halo_Bottom::print_iter_to_file(const char *filename) {
    if (this->rank_neighbor == MPI_PROC_NULL) {
        return;
    }
...
void Halo_Bottom::print_info(void) {
...
void Halo_Bottom::print_buff(double **buff, int m, int n) {
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
